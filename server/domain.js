...
export function calculateQuote(db, start, end, { checkPast = true } = {}) {
  ...
  if (checkPast && start < today())
    throw new AppError("O check-in não pode estar no passado.");
  const settings = JSON.parse(
    db.prepare("SELECT value FROM settings WHERE id = 1").get().value,
  );
  if (!settings.pricingEnabled)
    throw new AppError(
      "Tarifas ainda não publicadas. Consulte a anfitriã.",
      422,
    );
  const rules = db
    .prepare(
  ...
  const nightlyDetails = days.map((day) => {
    ...
    if (!rule)
      throw new AppError(
        "Não há tarifa cadastrada para todas as noites selecionadas.",
        422,
      );
  ...
  if (days.length < minNights)
    throw new AppError(
      `Este período exige no mínimo ${minNights} noites.`,
      422,
    );
...
export function assertAvailable(db, start, end, exceptId = "") {
  ...
  if (
    ...
    throw new AppError(
      "O período já está reservado ou bloqueado. Escolha outras datas.",
      409,
    );
}
